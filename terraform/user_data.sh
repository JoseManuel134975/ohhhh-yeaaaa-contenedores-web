...
echo -e "ServerRoot "/usr/local/apache2"" | sudo tee -a httpd.conf > /dev/null
...
echo -e "LoadModule mpm_event_module modules/mod_mpm_event.so
...
echo -e "LoadModule reqtimeout_module modules/mod_reqtimeout.so
...
echo -e "#LoadModule session_cookie_module modules/mod_session_cookie.so
...
echo -e "<Directory "/usr/local/apache2/cgi-bin">
    AllowOverride None
    Options None
    Require all granted
</Directory>
